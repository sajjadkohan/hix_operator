...
const ProductItem = ({data}) => {
  return (
    ...
    <Grid className={styles.parentAll} container spacing={1} size={12}>
        ...
        <Grid item size={2}><span className={`${styles.box} `}>{data.price}</span> </Grid>
        <Grid item size={2}><span className={styles.box2}>{data.category}</span></Grid>
        <Grid item size={2}><span className={styles.box3}>{data.count}</span></Grid>
